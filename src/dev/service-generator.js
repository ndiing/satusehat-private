...
function parseItem3(item3, names) {
    ...
    if (rawUrl) {
        ...
        if (!group[fileName]) {
            ...
            group[fileName] += `const Service = require("../service");\r\n`;
            group[fileName] += `\r\n`;
            group[fileName] += `class ${className} extends Service {\r\n`;
        }
        ...
        group[fileName] += `                method: "${method}",\r\n`;
        group[fileName] += `                headers: {\r\n`;
        // group[fileName] += `                    Authorization: "Bearer {{token}}",\r\n`;
        ...
        if (rawBody) {
            if (mode == "urlencoded") {
                // group[fileName] += `                body: new URLSearchParams(${JSON.stringify(rawBody, null, 4).replace(/^/gm, "                ").replace(/^\s+/, "")}).toString(),\r\n`.replace(/^/gm, "// ");
                // group[fileName] += `\r\n`;
                group[fileName] += `                body: new URLSearchParams(body).toString(),\r\n`;
            } else if (mode == "raw") {
                // group[fileName] += `                body: JSON.stringify(${JSON.stringify(rawBody, null, 4).replace(/^/gm, "                ").replace(/^\s+/, "")}),\r\n`.replace(/^/gm, "// ");
                // group[fileName] += `\r\n`;
                group[fileName] += `                body: JSON.stringify(body),\r\n`;
            }
        }
        group[fileName] += `            });\r\n`;
        group[fileName] += `            const json = response.json();\r\n`;
        group[fileName] += `            return json;\r\n`;
        group[fileName] += `        } catch (error) {\r\n`;
        group[fileName] += `            throw error;\r\n`;
        group[fileName] += `        }\r\n`;
        group[fileName] += `    }\r\n`;
...
for (const fileName in group) {
    ...
    fs.writeFileSync(`./src/api/satusehat/services/${fileName}.js`, group[fileName]);
}
